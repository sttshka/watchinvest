  ...
  <div class="overflow-hidden min-h-screen bg-gray-1 pb-10">
    ...
    <div class="bg-gray-1 px-4 lg:px-0">
      <div class="w-full adaptive-container">
        <AppPagination
          v-if="!searchStore.isError && jewelry.length"
        ...
        <div
          ...
          v-if="
            !searchStore.isError && jewelry.length && jewelry.length % 4 === 0
          "
        >
...
import JewelryList from "~/components/jewelry/JewelryList.vue";
import AppBreadcrumbs from "~/components/AppBreadcrumbs.vue";
import AppSearch from "~/components/AppSearch.vue";
import { IFilter, FilterType, Handbook, Entity, IJewelry } from "~/types";
...
const jewelry = ref<IJewelry[]>([]);
await handleSearch(true);
async function handleSearch(isInit = false) {
  const { items } = await searchStore.loadProducts<IJewelry>(
    Entity.jewelry,
    isInit
  );
  if (items) {
