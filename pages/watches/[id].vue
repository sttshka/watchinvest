<template>
      ...
      >
        <div class="flex justify-between text-white">
          <div class="flex flex-col gap-2">
            ...
            >
              USD {{ Number(item.investment_price).toLocaleString() }}
            </div>
          </div>
          <div
        ...
        <div class="flex items-center gap-4 w-full">
          <AppButton
            ...
            :to="
              cartStore.checkProductInCart(item.id, Entity.watches)
                ? '/cart'
                : null
            "
            @click.once="handleAddToCartAndRedirect"
          >
            Buy now
          </AppButton>
          <AppButton
            ...
            color="secondary"
            class="h-[60px] flex-1 !max-w-full"
            :disabled="cartStore.checkProductInCart(item.id, Entity.watches)"
          ...
          >
            <template
              v-if="!cartStore.checkProductInCart(item.id, Entity.watches)"
    ...
    >
      <div class="lg:max-w-[320px] xl:max-w-[392px] 2xl:max-w-[472px] w-full">
        <div class="flex flex-col gap-3 lg:gap-4">
          ...
          <div class="text-secondary text-[12px] lg:text-[14px] leading-[150%]">
        ...
        <div class="flex flex-col gap-2 lg:gap-3 xl:gap-4 2xl:gap-6">
          ...
          <div class="flex flex-col gap-2 xl:gap-3 2xl:gap-4">
            ...
            >
              ...
              <div v-if="location" class="flex-1 text-gray-text">
                {{ location }}
              </div>
              <div v-else class="flex-1 text-gray-text">–</div>
            </div>
        ...
        <div class="flex flex-col gap-2 xl:gap-4 2xl:gap-6">
          ...
          <div class="flex flex-col gap-2 xl:gap-3 2xl:gap-4">
            <div
              class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
            >
              <div class="flex-1">Case Diameter</div>
              <div v-if="item.body_diameter" class="flex-1 text-gray-text">
                {{ item.body_diameter }}mm
              </div>
              <div v-else class="flex-1 text-gray-text">–</div>
            </div>
            <div
              class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
            >
              <div class="flex-1">Case material</div>
              <div v-if="item.body_material" class="flex-1 text-gray-text">
                {{ item.body_material.value_en }}
              </div>
              <div v-else class="flex-1 text-gray-text">–</div>
            </div>
            <div
              class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
            >
              <div class="flex-1">Bracelet material</div>
              <div v-if="item.strap_material" class="flex-1 text-gray-text">
                {{ item.strap_material.value_en }}
              </div>
              <div v-else class="flex-1 text-gray-text">–</div>
            </div>
            <div
              class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
            >
              <div class="flex-1">Crystal</div>
              <div v-if="item.caliber" class="flex-1 text-gray-text">
                {{ item.caliber }}
              </div>
              <div v-else class="flex-1 text-gray-text">–</div>
            </div>
            ...
            <div
              class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
            >
              <div class="flex-1">Movement</div>
              <div v-if="item.mechanism_type_en" class="flex-1 text-gray-text">
                {{ item.mechanism_type_en }}
              </div>
              <div v-else class="flex-1 text-gray-text">–</div>
            </div>
            ...
            <div
              class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
            >
              <div class="flex-1">Power reserve</div>
              <div v-if="item.power_reserve" class="flex-1 text-gray-text">
                {{ item.power_reserve }} h
              </div>
              <div v-else class="flex-1 text-gray-text">–</div>
            </div>
            <div
              class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
            >
              <div class="flex-1">Year of Production</div>
              <div class="flex-1 text-gray-text" v-if="item.atime">
                {{ useDateFormat(item.atime, "YYYY").value }}
              </div>
              <div v-else class="flex-1 text-gray-text">–</div>
            </div>
            <div
              class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
            >
              <div class="flex-1">Completeness</div>
              <div class="flex-1 text-gray-text" v-if="item.count">38</div>
              <div v-else class="flex-1 text-gray-text">–</div>
            </div>
            <div
              class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
            >
              <div class="flex-1">Water Resistance</div>
              <div class="flex-1 text-gray-text" v-if="item.water_resistance">
                {{ item.water_resistance }} meters
              </div>
              <div v-else class="flex-1 text-gray-text">–</div>
            </div>
            <div
            ...
            >
              ...
              <div class="flex-1 text-gray-text" v-if="item.gender_en">
                {{ item.gender_en }}
              </div>
              <div v-else class="flex-1 text-gray-text">–</div>
            </div>
            <div
              class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
            >
              <div class="flex-1">Condition</div>
              <div class="flex-1 text-gray-text" v-if="item.condition_en">
                {{ item.condition_en }}
              </div>
              <div v-else class="flex-1 text-gray-text">–</div>
            </div>
        ...
        <div class="flex flex-col gap-2 xl:gap-4 2xl:gap-6">
          ...
          >
            Audemars Piguet Royal Oak double balance wheel mounts a movement of
            the highest manufacture with a new patented Dualbalance extremely
            complex structure that makes this timepiece precise and stable.
            Available with 18K rose gold case, sapphire crystal caseback, 41mm
            skeletonized dial and 18kt rose gold bracelet.
          </div>
          <div v-else class="flex-1 text-gray-text">–</div>
        </div>
      </div>
      <div
        class="lg:max-w-[320px] xl:max-w-[392px] 2xl:max-w-[472px] w-full flex flex-col gap-8"
      >
        <AppProductInvest :price="item.investment_price" />
        <AppChart
          :pricesHistory="pricesHistory"
          :chronoHistory="chronoHistory"
        />
        <AppTable
          :titles="['Time', 'Chrono24', 'WatchCharts']"
          :tableData="tableData"
        />

        <AppRetail
          :price="chronoPrice"
          :secondPrice="watchPrice"
          ce="chronoPrice"
        />
      </div>
    </div>
  ...
  <ClientOnly>
    ...
    <Teleport to="body">
      <AppModal v-model="isShowSuccessCopy" @close="isShowSuccessCopy = false">
        <p class="text-center">The link has been copied successfully</p>
      </AppModal>
    </Teleport>
  </ClientOnly>
</template>
...
const pricesHistory = ref([]);
const chronoHistory = ref([]);
const chronoPrice = ref("");
const watchPrice = ref("");

const { id } = route.params;

const tableData = ref<{ period: string; chrono: string; watch: string }[] | []>(
  []
);

const isShowSuccessCopy = ref(false);
...
const productStore = useWatchesStore();
const favoritesStore = useFavoritesStore();

const searchStore = useNewSearchStore();
const item = ref<IWatch>();
...
async function handleAddToCart() {
  if (cartStore.checkProductInCart(item.value.id, Entity.watches)) {
    return;
  }

  if (item.value) {
    await cartStore.addToCart(item.value.id, Entity.watches);
  }
}

async function handleAddToCartAndRedirect() {
...
async function handleLoadProduct() {
  const res = await searchStore.loadProduct<IWatch>(
    Entity.watches,
    id as string
  );
...
const shareLink = async () => {
  ...
  isShowSuccessCopy.value = true;
};

const calculate = (start: string, end: string | undefined) => {
  if (end === "undefined") return "-";
  return String(((+end / +start) * 100).toFixed(1));
};

const getPricesHistory = async () => {
  const { data } = await useApiFetch(`price_history/${item.value?.id}`);
  // const { data } = await useFetch(`https://watchapi.relabs.ru/api/v1/price_history/${item.value?.id}`)
  if (data?.value) {
    console.log({ data });

    pricesHistory.value = data?.value?.data.watchchart_history || {};
    chronoHistory.value = data?.value?.data.chrono_history || {};
    chronoPrice.value = String(
      Object.values(chronoHistory.value)[
        Object.values(chronoHistory.value).length - 1
      ]
    );
    watchPrice.value = String(
      Object.values(pricesHistory.value)[
        Object.values(pricesHistory.value).length - 1
      ]
    );
    tableData.value[0] = {
      period: "1M",
      chrono: calculate(
        String(Object.values(data?.value?.data.chrono_history)[0]),
        String(Object.values(data?.value?.data.chrono_history)[29])
      ),
      watch: calculate(
        String(Object.values(data?.value?.data.watchchart_history)[0]),
        String(Object.values(data?.value?.data.watchchart_history)[29])
      ),
    };
    tableData.value[1] = {
      period: "3M",
      chrono: calculate(
        String(Object.values(data?.value?.data.chrono_history)[0]),
        String(Object.values(data?.value?.data.chrono_history)[87])
      ),
      watch: calculate(
        String(Object.values(data?.value?.data.watchchart_history)[0]),
        String(Object.values(data?.value?.data.watchchart_history)[87])
      ),
    };
    tableData.value[2] = {
      period: "6M",
      chrono: calculate(
        String(Object.values(data?.value?.data.chrono_history)[0]),
        String(Object.values(data?.value?.data.chrono_history)[174])
      ),
      watch: calculate(
        String(Object.values(data?.value?.data.watchchart_history)[0]),
        String(Object.values(data?.value?.data.watchchart_history)[174])
      ),
    };
    tableData.value[3] = {
      period: "1Y",
      chrono: calculate(
        String(Object.values(data?.value?.data.chrono_history)[0]),
        String(Object.values(data?.value?.data.chrono_history)[365])
      ),
      watch: calculate(
        String(Object.values(data?.value?.data.watchchart_history)[0]),
        String(Object.values(data?.value?.data.watchchart_history)[365])
      ),
    };
  }
};
