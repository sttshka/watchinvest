...
import { tryResolveModule, resolvePath, useLogger, extendPages, addWebpackPlugin, extendWebpackConfig, addVitePlugin, extendViteConfig, createResolver, addServerPlugin, defineNuxtModule, isNuxt2, getNuxtVersion, isNuxt3, addPlugin, addTemplate, addComponent, addImports } from '@nuxt/kit';
import { resolve, parse, join, normalize, relative, isAbsolute, dirname } from 'pathe';
...
function setupPages(options, nuxt, additionalOptions = {
  ...
  debug$8(`pagesDir: ${pagesDir}, srcDir: ${srcDir}, tailingSlash: ${trailingSlash}`);
  extendPages((pages) => {
...
async function setupNitro(nuxt, nuxtOptions, nuxtI18nOptionsCode) {
...
const debug$2 = createDebug("@nuxtjs/i18n:gen");
...
function simplifyLocaleOptions(nuxt, options) {
  const hasObjectLocales = nuxt.options._layers.some((layer) => layer?.config?.i18n?.locales?.some((x) => typeof x !== "string")) || options?.i18nModules?.some((module) => module?.locales?.some((x) => typeof x !== "string"));
...
function generateLoaderOptions(nuxt, { nuxtI18nOptions, vueI18nConfigPaths, localeInfo, isServer }) {
  debug$2("generateLoaderOptions: lazy", nuxtI18nOptions.lazy);
  ...
  debug$2("generate code", generated);
  return generated;
}
...
${options.localeMessages.map(([key, val]) => {
    return `  "${key}": [${val.map(
