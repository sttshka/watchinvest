        ...
        >
          <div class="flex justify-between text-white">
            <div class="flex flex-col gap-2">
              ...
              >
                <template v-if="item.purchase_price">150 BTC</template>
          ...
          <div class="flex items-center gap-4 w-full">
            ...
            <AppButton
              ...
              :disabled="cartStore.checkProductInCart(item.id, Entity.jewelry)"
              :loading="cartStore.productsIsLoading"
              :to="
                cartStore.checkProductInCart(item.id, Entity.jewelry)
                  ? '/cart'
                  : null
              "
            ...
            >
              <template
                v-if="!cartStore.checkProductInCart(item.id, Entity.jewelry)"
          ...
          <div class="flex flex-col gap-2 xl:gap-4 2xl:gap-6">
            ...
            <div class="flex flex-col gap-2 xl:gap-3 2xl:gap-4">
              <div
                class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
              >
                <div class="flex-1">Carat</div>
                <div v-if="item.carat" class="flex-1 text-gray-text">
                  {{ item.carat }}
                </div>
                <div v-else class="flex-1 text-gray-text">–</div>
              </div>
              <div
                class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
              >
                <div class="flex-1">Inserts Count</div>
                <div v-if="item.inserts_count" class="flex-1 text-gray-text">
                  {{ item.inserts_count }}
                </div>
                <div v-else class="flex-1 text-gray-text">–</div>
              </div>
              <div
                class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
              >
                <div class="flex-1">Jewelry Size</div>
                <div v-if="item.jsize" class="flex-1 text-gray-text">
                  {{ item.jsize }}
                </div>
                <div v-else class="flex-1 text-gray-text">–</div>
              </div>
              <div
                class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
              >
                <div class="flex-1">Jewelry Count</div>
                <div v-if="item.jcount" class="flex-1 text-gray-text">
                  {{ item.jcount }}
                </div>
                <div v-else class="flex-1 text-gray-text">–</div>
              </div>
              ...
              >
                ...
                <div v-if="item.weight" class="flex-1 text-gray-text">
                  {{ item.weight }}
                </div>
                <div v-else class="flex-1 text-gray-text">–</div>
              </div>
              <div
                class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
              >
                <div class="flex-1">Height</div>
                <div v-if="item.height" class="flex-1 text-gray-text">
                  {{ item.height }}
                </div>
                <div v-else class="flex-1 text-gray-text">–</div>
              </div>
              <div
                class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
              >
                <div class="flex-1">Len</div>
                <div v-if="item.len" class="flex-1 text-gray-text">
                  {{ item.len }}
                </div>
                <div v-else class="flex-1 text-gray-text">–</div>
              </div>
              ...
              <div
                class="flex text-[14px] lg:text-[15px] 2xl:text-base !leading-[150%]"
              >
                <div class="flex-1">Material</div>
                <div v-if="item.material_id" class="flex-1 text-gray-text">
                  {{ item.material_id }}
                </div>
                <div v-else class="flex-1 text-gray-text">–</div>
              </div>
              ...
              >
                ...
                <div v-if="item.color_id" class="flex-1 text-gray-text">
                  {{ item.color_id }}
                </div>
                <div v-else class="flex-1 text-gray-text">–</div>
              </div>
...
const item = ref<IJewelry>();
await handleLoadProduct();
modifyBreadcrumbs();

const cartStore = useCartStore();
async function handleAddToCart() {
  if (cartStore.checkProductInCart(item.value.id, Entity.jewelry)) {
    return;
  }
  if (item.value) {
    await cartStore.addToCart(item.value.id, Entity.jewelry);
  }
}

async function handleLoadProduct() {
  const res = await searchStore.loadProduct<IJewelry>(
    Entity.jewelry,
    id as string
  );
...
function modifyBreadcrumbs() {
  const newCrumbs = [
    ...
    {
      title: item.value?.article,
      to: `/jewelry/${item.value?.id}`,
    },
  ];
